...
# ── Adjust sys.path so `webpilot` package resolves ───────────────────────────
...
def _prompt(msg: str, password: bool = False) -> str:
    if HAS_RICH:
        return Prompt.ask(msg, password=password)
    else:
        if password:
            import getpass
...
def print_result(result: dict):
    if result["success"]:
        ...
        msg = (
            ...
            f"Partial: {result.get('partial_result', 'None')}"
        )
        if HAS_RICH:
...
async def interactive_loop():
    ...
    if not api_key:
        api_key = _prompt("Enter your Anthropic API key", password=True)

    headless = not _confirm("Show browser window?", default=True)

    _print("\n⚙️  Starting WebPilot...", "yellow")
    agent = WebPilotAgent(api_key=api_key, headless=headless)
    ...
    try:
        ...
        await agent.stop()
        _print("\n👋 WebPilot stopped. Good luck at the hackathon!", "yellow")
