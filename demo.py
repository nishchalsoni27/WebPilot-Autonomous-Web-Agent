#!/usr/bin/env python3
"""
WebPilot Demo Script (Local Deterministic Sandbox Execution Mode)
...
DEMOS = [
    {
        ...
        "task": "Search for 'top AI breakthroughs 2026' on Google, open the first result, and summarise breakthroughs.",
        "highlights": ["Google navigation", "Link following", "Content extraction"],
    },
    {
        "id": 2,
        "name": "📰  News Aggregation",
        "task": "Go to https://news.ycombinator.com and list the top 5 story titles along with scores.",
        "highlights": ["Direct navigation", "Structured data extraction"],
    },
    {
        "id": 3,
        "name": "🛒  E-commerce Research",
        "task": "Go to https://www.flipkart.com, search for 'wireless headphones under 2000' and extract results.",
        "highlights": ["E-commerce navigation", "Search", "Price/rating extraction"],
    },
    {
        "id": 4,
        "name": "📚  Knowledge Retrieval",
        "task": "Go to Wikipedia and look up 'Large Language Model'.",
        "highlights": ["Wikipedia navigation", "Multi-fact extraction"],
    },
    {
        "id": 5,
        "name": "✈️  Travel Price Research",
        "task": "Go to https://www.makemytrip.com, search for flights from Delhi to Mumbai.",
        "highlights": ["Multi-step interaction", "Price comparison"],
    },
]
...
def print_result(result: dict, demo_name: str):
    ...
    if result["success"]:
        ...
        print(f"\nStats — Steps: {result.get('steps',0)} | Time: {result.get('elapsed_s',0)}s")
    else:
...
async def run_demo(demo: dict, headless: bool = False, save: bool = True) -> dict:
    ...
    agent = WebPilotAgent(headless=headless, max_steps=10)
...
async def run_all_demos(headless: bool = True):
    print("\n" + "=" * 64)
    print("  WebPilot — Full Demo Suite (Local Sandbox)")
    print("=" * 64)
    ...
    for demo, res in results:
        ...
        print(f"  {demo['id']:<4} {demo['name']:<30} {status}")
    print("=" * 64)


def interactive_menu():
    print("\n" + "=" * 64)
    print("  WebPilot Demo Selector")
    print("=" * 64)
    for d in DEMOS:
        print(f"  {d['id']}. {d['name']}")
...
def main():
    ...
    parser.add_argument("--task", type=int, choices=range(1, len(DEMOS) + 1), help="Run a specific demo")
